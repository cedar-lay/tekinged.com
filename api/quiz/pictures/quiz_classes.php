<?php
/*
 * api/quiz/pictures/quiz_classes.php
 *
 * Shared logic for the JSON-based Pictures quiz API.
 * Adapted from John's quiz2.php + pics.php.
 *
 * The user is shown a picture and picks the correct word from 5
 * options. Question generation uses the same weighted-random-category
 * selection pattern as the Vocab quiz (pick a tag, weighted by how
 * many qualifying pictures share it, then pull 5 words from it).
 *
 * Same "last row wins" correct-answer quirk as Proverbs/Audio — see
 * the note in api/quiz/proverbs/quiz_classes.php for why this is
 * faithful to live behavior rather than a bug needing a fix.
 *
 * Image files are static and predictable: /uploads/pics/{id}.jpg
 * (matches pics_dir() from functions.php). NOTE: as of this writing,
 * pictures exist only on the live production server, not staging —
 * the front-end JS points image URLs at the live domain to match
 * (same workaround already used on the Wordlist page).
 *
...
function quizlog($mysqli, $pword, $correct, $qtype = 'Pictures') {
...
function quizzedlog($mysqli, $name, $correct, $total, $qtype = 'Pictures') {
    list($s, $p, $u, $a) = scrape_ip();
...
class PictureQuestion {
    public $myid;
    public $imageUrl;
    public $labels;       // 5 candidate words (shuffled), for display
    public $values;       // matching 5 database ids, same order as labels
    public $correctValue;
    public $correctLabel; // the word the picture actually shows

    public function __construct($myid, $imageUrl, $labels, $values, $correctValue, $correctLabel) {
        $this->myid = $myid;
...
/**
 * Generates one Pictures question, mirroring pic_make_question() from
 * pics.php: picks a random tag/category weighted by how many
 * qualifying pictures share it, then 5 random words from that tag as
 * the multiple-choice options (last row wins as the correct answer —
 * see file-level note above).
 */
function generate_picture_question($mysqli, $quiz) {
    $qfilter = $quiz->get_filter();
    $filter = "!isnull(tags) && $qfilter && id in (select allwid from pictures where (uploaded=1 || uploaded=2))";
    $table = 'qz_pics';

    // Weighted-random tag selection (same pattern as the Vocab quiz's
    // weighted part-of-speech selection).
    $all_tags = [];
    $tag_q = "select tags,count(*) as c from $table group by tags having c>=5";
    $r = query_or_die($mysqli, $tag_q);
    $total = 0;
    ...
    $rand = rand(0, $total);
    $tag = 'fish'; // fallback default, matches original
    ...
    $q_q = "select pal,id from $table where tags like '$tagEsc' and $filter order by rand() limit 5;";
    ...
    $myid = $quiz->asked() + 1;
    $imageUrl = '/uploads/pics/' . $correctId . '.jpg';

    return new PictureQuestion($myid, $imageUrl, $labels, $values, $correctId, $correctPal);
}
